...
'''
Docstring for project.correlation.correlation
This code finds the Pearson correlation between genres in the 
Billboard Hot 100 Chart data from 1958-2021 
and displays the Clustered Heatmap.
'''
...
# Global Variables 
INPUT_CSV = "../../data/BIG_DATA2.csv" # Input csv must be of format: 
...
pivot_filename = 'genre_proportions_by_' + AGG_PERIOD + '_top_' + str(TOP_N_GENRES) + str(INPUT_CSV) 
proportion_table_top.to_csv(pivot_filename) # Save pivot table, output to csv
print("\nSaved to {}".format(pivot_filename))
...
correlation_filename = 'correlation_by_' + AGG_PERIOD + '_top_' + str(TOP_N_GENRES) + str(INPUT_CSV) 
correlation.to_csv(correlation_filename) # Save to csv
print("\nSaved to {}".format(correlation_filename))
...
plt.suptitle('Genre Correlation Clustered Heatmap by {} Top {} Genres'.format(str(AGG_PERIOD), str(TOP_N_GENRES)))
fig_filename = 'clustermap_by_' + AGG_PERIOD + '_top_' + str(TOP_N_GENRES) + str(INPUT_CSV) + '.png'
